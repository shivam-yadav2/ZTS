                                    ...
                                    <tr>
                                        <td>
                                            <div class="flex items-center">
                                                <img src="{{ asset('assets/uploads/homeslider/' . $value->img) }}"
                                        ...
                                        <td>
                                            <div class="flex items-center">
                                                <img src="{{ asset('assets/uploads/homeslider/' . $value->mobile_img) }}"
                                                    alt="" class="shrink-0 me-3 rounded-lg"
                                                    style="width:120px; height:70px;">
                                        ...
                                        <td class="text-center">
                                            <label class="switch inline-flex items-center cursor-pointer">
                                                <input type="checkbox" class="sr-only peer toggle-status"
                                                    data-id="{{ $value->id }}" {{ $value->is_active ? 'checked' : '' }}>
                                                <span
                                                ...
                                                <span
                                                    class="line-height-1 font-medium ms-3 peer-checked:text-primary-600 text-md text-gray-600 dark:text-gray-300">{{ $value->is_active ? 'Active' : 'Inactive' }}</span>
                                            </label>
                                        </td>

                                        <td>
                                            <a href="{{ url('admin/slider/destroy/' . $value->id) }}"
                                                onclick="return confirm('Are you sure you want to delete this slider?')"
                                                class="text-red-600 hover:text-red-800"> <i
                                                    class="fa-solid fa-trash"></i></a>
                                            <a href="{{ url('admin/slider/edit/' . $value->id) }}"
                                                class="ms-3 text-blue-600 hover:text-blue-800"><i
    ...
    <script>
        document.querySelectorAll('.toggle-status').forEach(toggle => {
            toggle.addEventListener('change', function() {
                ...
                'span:last-child'); // for updating status text
